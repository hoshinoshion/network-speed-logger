...
- One-click cleanup that moves every item in the output folder to the system Recycle Bin or Trash after confirmation.
...
Windows Settings can enable a non-interactive real-time upload and download speed display on the primary taskbar. The default two-line layout shows upload first and download second; an optional single-line layout places them left to right with comfortable spacing. It has its own automatic or manual adapter selection, sample interval (one second by default), and Byte/bit unit. The B, K, M, G, and T prefix changes automatically with the current speed. With centered Start and app buttons, the display sits on the left immediately after the Windows Widgets area; with left-aligned buttons, it sits on the right before the notification area. It uses the modern Windows 11 system typeface when available, follows the taskbar theme including high-contrast mode, and hides with an auto-hidden taskbar or a full-screen application.
