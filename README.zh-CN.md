...
“设置”中还可以开启 macOS 的“开机运行”。开启时会自动开启状态栏后台运行；由 macOS 登录触发的启动会直接进入状态栏，不打开主窗口。关闭后会注销登录项。

另一个独立设置可以让状态栏项目在应用整个运行期间常驻，并在箭头左侧以上传在上、下载在下的双行形式显示实时网速。它拥有独立的自动/手动网络接口选择、采样频率（默认 1 秒）和 Byte/bit 单位，B、K、M、G、T 前缀会随当前网速动态变化。上传与下载箭头会分别在对应速度达到可设置的活动阈值时完全不透明，低于阈值时显示为半透明；默认阈值为 50 KB/s。启用实时显示不会改变独立的关窗与 Dock 行为。
...
还可以在“设置”中开启最小化到系统托盘。开启后点击关闭按钮只会隐藏窗口，不会中断正在进行的记录，并会短暂显示 Windows 系统通知。左键点击托盘图标可重新打开窗口；右键菜单可开始记录、停止记录或退出应用。

“开机运行”使用当前 Windows 用户的标准启动注册项。开启时也会自动开启托盘后台运行，登录系统后应用会直接进入托盘；关闭后会删除启动注册项。

Windows“设置”中还可以开启不可交互的任务栏实时网速显示，并通过下拉菜单选择双行或单行布局。双行布局第一行显示上传、第二行显示下载；单行布局使用更大的字体，让上传和下载从左到右显示并保持适度间距。两种布局均使用常规字重的系统字体和高质量灰度边缘。它拥有独立的自动或手动网卡模式、网卡选择、采样频率（默认为 1 秒）以及 Byte/bit 单位；B、K、M、G、T 等前缀会根据实时速度自动变化。开始按钮和应用图标居中时，网速显示位于任务栏左侧并紧跟 Windows 小组件区域；按钮靠左时，网速显示位于通知区域前方并靠右。显示会优先使用 Windows 11 的现代系统字体，文字对比度会适配任务栏的浅色、深色和高对比度主题。显示窗口绑定到任务栏自身，在开始菜单、通知中心、托盘、桌面和普通软件之间切换时会保持显示，并在任务栏自动隐藏时一同隐藏。
...
```sh
bash src/macos/NetworkSpeedLogger/Scripts/build-release.sh 1.0.0 44
```
