...
Windows“设置”中还可以开启不可交互的任务栏实时网速显示。它拥有独立的自动或手动网卡模式、网卡选择、采样频率（默认为 1 秒）以及 Byte/bit 单位；B、K、M、G、T 等前缀会根据实时速度自动变化。开始按钮和应用图标居中时，网速显示位于任务栏左侧；按钮靠左时，网速显示位于通知区域前方并靠右。显示会优先使用 Windows 11 的现代系统字体，文字对比度会适配任务栏的浅色、深色和高对比度主题，并在任务栏自动隐藏或全屏应用运行时一同隐藏。
